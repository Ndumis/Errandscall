...
namespace Errandscall.CustomAuthentication
{
    public class CustomRole : RoleProvider
    {
        ...
        public override string[] GetRolesForUser(string username)
        {
            ...
            {
                var selectedUser = (from us in dbContext.Login
                                    where string.Compare(us.Client.Email, username, StringComparison.OrdinalIgnoreCase) == 0
                                    select us).FirstOrDefault();
                
                if(selectedUser != null)
                {
                    userRoles = new[] { selectedUser.UserRoleId.ToString() };
                }
